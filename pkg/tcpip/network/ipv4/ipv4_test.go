...
func compareFragments(t *testing.T, packets []*stack.PacketBuffer, sourcePacketInfo *stack.PacketBuffer, mtu uint32) {
	...
	v, vs := sourcePacketInfo.Views()
	source = append(source, v...)
	source = append(source, vs.ToView()...)
	...
	for i, packet := range packets {
		...
		gotHdr, _ := packet.Views()
		wantHdr, _ := sourcePacketInfo.Views()
		if got, want := len(gotHdr), len(wantHdr)+header.IPv4MinimumSize; i == 0 && want < int(mtu) && got != want {
			t.Errorf("first fragment hdr parts should have unmodified length if possible: got %d, want %d", got, want)
		}
		if got, want := packet.LinkHeader.AvailableLength(), sourcePacketInfo.LinkHeader.AvailableLength()-header.IPv4MinimumSize; got != want {
